...
pub mod cpu {
    ...
    pub fn macos() -> String {
        ...
        return std::str::from_utf8(&sysctl.output().map_or(Vec::from("unknown"), |x| x.stdout))
            .unwrap()
            .trim()
            .to_string();
    }
...
pub mod reporter {
    ...
    pub fn br(options: &Options) -> String {
        ...
        s.push_str(
            &"-".repeat(options.size + 14 * options.avg as usize + 24 * options.min_max as usize),
        );
    ...
    pub fn benchmark(name: &str, stats: &BenchmarkStats, options: &Options) -> String {
        ...
        if !options.colors {
            if options.avg {
                s.push_str(&format!(
                    "{:>14}",
                    format!("{}/iter", fmt::duration(stats.avg))
                ));
            }
            if options.min_max {
                s.push_str(&format!(
                    ...
                    format!(
                        "({} … {})",
                        fmt::duration(stats.min),
                        fmt::duration(stats.max)
                    )
                ));
            }
            if options.percentiles {
                s.push_str(&format!(
                    " {:>9} {:>9} {:>9}",
                    fmt::duration(stats.p75),
                    fmt::duration(stats.p99),
                    fmt::duration(stats.p995)
                ));
            }
        } else {
            if options.avg {
                s.push_str(&format!(
                    ...
                    format!(
                        "{}/iter",
                        fmt::color(&fmt::duration(stats.avg), fmt::Color::Yellow)
                    )
                ));
            }
            if options.min_max {
                s.push_str(&format!(
                    ...
                    format!(
                        "({} … {})",
                        fmt::color(&format!("{}", fmt::duration(stats.min)), fmt::Color::Cyan),
                        fmt::color(
            ...
            if options.percentiles {
                s.push_str(&format!(
                    " {:>18} {:>18} {:>18}",
                    fmt::color(&format!("{}", fmt::duration(stats.p75)), fmt::Color::Gray),
                    fmt::color(&format!("{}", fmt::duration(stats.p99)), fmt::Color::Gray),
                    fmt::color(&format!("{}", fmt::duration(stats.p995)), fmt::Color::Gray)
                ));
            }
        }
    ...
    pub fn summary(benchmarks: &[GroupBenchmark], options: &Options) -> String {
        ...
        if !options.colors {
            ...
            for b in benchmarks.iter().filter(|b| *b != baseline) {
                ...
                let diff = f64::from_str(&format!("{:.2}", 1.0 / baseline.stats.avg * b.stats.avg))
                    .unwrap();
                let inv_diff =
                    f64::from_str(&format!("{:.2}", 1.0 / b.stats.avg * baseline.stats.avg))
                        .unwrap();
                s.push_str(&format!(
                    "\n   {}x times {} than {}",
            ...
            s.push_str(&format!(
                "{}\n  {}",
                fmt::bold("summary"),
                fmt::bold(&fmt::color(&baseline.name, fmt::Color::Cyan))
            ));
            ...
            for b in benchmarks.iter().filter(|b| *b != baseline) {
                ...
                s.push_str(&format!(
                    "\n   {}x {} than {}",
                    if faster {
                        fmt::color(&format!("{}", diff), fmt::Color::Green)
                    } else {
                        fmt::color(&format!("{}", inv_diff), fmt::Color::Red)
                    },
                    if faster { "faster" } else { "slower" },
                    fmt::bold(&fmt::color(&b.name, fmt::Color::Cyan))
                ));
            }
        }
...
pub mod bench {
        ...
        F: Fn() -> T,
    {
...

        {
            ...
            loop {
                if 0 > iterations && budget == std::time::Duration::from_millis(0) {
                    break;
                }
...
pub fn run_benchmarks(benchmarks: Vec<String>) -> Vec<reporter::GroupBenchmark> {
    let mut options =
    ...
    for cmd in benchmarks {
        ...
        let stats = reporter::BenchmarkStats {
            avg: stats.avg,
            min: stats.min,
            max: stats.max,
            p75: stats.p75,
        ...
        results.push(reporter::GroupBenchmark {
            name: cmd,
            stats: stats,
            baseline: false,
